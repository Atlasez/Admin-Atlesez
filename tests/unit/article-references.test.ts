...
describe("article references", () => {
  ...
  it("preserves uppercase identifiers while accepting case-insensitive citations", () => {
    expect(
      normalizeArticleReferences([{ id: "GrothEndieck", title: "EGA" }]),
    ).toEqual([{ id: "GrothEndieck", title: "EGA" }]);
    expect(
      citedReferenceIds("[[cite:grothendieck]] and [[cite:GrothEndieck]]."),
    ).toEqual(["grothendieck"]);
  });

  it("extracts citation identifiers in first-use order", () => {
    expect(
      citedReferenceIds(
        "[[cite:first]] and [[cite:second]] then [[cite:first]].",
      ),
  ...
  it("formats a bibliography entry and anchor", () => {
    ...
    ).toBe("Author. Algebra (2020). Press");
