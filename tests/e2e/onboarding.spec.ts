...
test("応募フォームの基本情報にプロジェクト選択を置かず、個人情報案内と表示設定を表示する", async ({ page }) => {
  await page.goto("apply/");
  await expect(page.getByRole("group", { name: "基本情報" })).toBeVisible();
  await expect(page.getByLabel("応募先プロジェクト")).toHaveCount(0);
  await expect(page.locator(".privacy-notice")).toContainText("個人情報の取り扱いについて");
...
test("基本情報とプロジェクト情報を別ページで入力し、マイページへ進める", async ({ page }) => {
  let savedBasicProfile: Record<string, string> | undefined;
  await page.route("**/api/onboarding/me", async (route) => {
    if (route.request().method() === "POST") {
      savedBasicProfile = route.request().postDataJSON() as Record<string, string>;
  ...
  await page.route("**/api/onboarding/project", async (route) => {
    if (route.request().method() === "POST") {
      await route.fulfill({ json: { ok: true, next: "/onboarding/tutorial/" } });
      return;
    }
    await route.fulfill({
      json: {
  ...
  await expect(page.getByRole("heading", { name: "基本情報を設定してください" })).toBeVisible();
  ...
  await page.getByRole("button", { name: "基本情報を保存して次へ進む" }).click();
  await expect.poll(() => savedBasicProfile).toMatchObject({
    displayName: "山田 花子",
    bio: "よろしくお願いします。",
  });

  await page.goto("onboarding/project/");
  await expect(page.getByRole("heading", { name: "プロジェクトの情報を設定してください" })).toBeVisible();
  await expect(page.getByRole("link", { name: "マイページ" })).toHaveAttribute("href", "/admin/member-profile/");
