...
test("目次APIのDB列名を画面モデルへ正規化して表示する", async ({ page }) => {
  await mockAdminApi(page, undefined, undefined, [], [
    {
      id: "outline-db-1",
  ...
  await page.goto("./admin/editor/outline/?subject=mathematics&category=group-theory");
  await expect(
    page.locator("[data-outline-list] .outline-entry-title").filter({ hasText: "集中不等式" }),
  ).toHaveCount(1);
  await expect(
    page.locator("[data-outline-list] .outline-entry-meta").filter({ hasText: "concentration-test" }),
  ).toHaveCount(1);
});
