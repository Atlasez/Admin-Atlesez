...
操作履歴と権限変更履歴は、公開・承認・権限変更などの証跡としてD1に保存します。保持期限は作成から730日です。期限を過ぎた行は定期処理で`archived_at`を付与するだけで、物理削除や内容の書き換えは行いません。

通常の一覧は未アーカイブの履歴だけを表示します。管理者が過去の証跡を確認する場合は、操作履歴または権限変更履歴の「アーカイブを含む」を有効にしてください。アーカイブ済みの行は表示上区別されますが、通常の検索・カーソルページネーションと同じ条件で参照できます。

保持期間やアーカイブ方式を変更する場合は、`migrations/0102_admin_audit_log_archives.sql`と`AUDIT_LOG_RETENTION_DAYS`を同じPRで更新し、既存行を削除しないこと、復旧方法、D1容量への影響をPR本文に記録します。
