...
数学ブロックは`:::`または`::::`のコンテナdirectiveで境界を明示します。記事編集画面では
本文欄上部の「証明」「折りたたみ」「注釈」から同じ記法を挿入でき、公開サイトと
同じ境界でプレビューします。1個・2個・5個以上のコロンは、本文との衝突や過剰な
入れ子を避けるため使いません。
...
入れ子では外側を`::::`、内側を`:::`のように、外側のコロンを長くします。
