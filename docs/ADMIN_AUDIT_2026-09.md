...
Chromeのモック認証セッションで、ポータル・横断タスク・アクションセンター・監査ログを、1440/768/390pxの各幅とライト/ダーク/ブラックの各表示で撮影した。代表画像は作業環境の`/private/tmp/atlasez-goal-screens/`に保存している。全36枚で横スクロール、白い大面積サーフェス、カードの重なりは確認されなかった。
