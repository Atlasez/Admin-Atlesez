...
function trustedPreviewOrigin(
  ...
  if (candidate.protocol !== "https:" || candidateHost === publicHost)
    return null;
...
async function completeSearchConsoleImport(
  ...
  const returnOrigin =
    trustedPreviewOrigin(savedState.returnOrigin, request, env) ??
    adminPublicOrigin(request, env);
  const reportsRedirect = (result: string) =>
    Response.redirect(`${returnOrigin}/admin/reports/?gsc=${result}`, 302);
  ...
  if (!token.access_token) return reportsRedirect("error");
...
async function logoutAdmin(request: Request, env: Env): Promise<Response> {
  ...
  headers.append(
    "set-cookie",
    cookie(ADMIN_SESSION_COOKIE, "", 0, "/", authCookieDomain(request, env)),
  );
  return new Response(null, { status: 302, headers });
}
