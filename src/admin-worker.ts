...
async function listEditorialDocuments(
  ...
  return json({
    ...
    mentionNames: (memberRows.results ?? []).map((member) =>
      member.display_name.trim() || member.email.split("@")[0],
    ),
    scope: {
      email: scope.email,
...
async function getEditorialDocument(
  ...
  const latestFeedback = new Map<string, Map<string, "acknowledge" | "unacknowledge">>();
  for (const action of actionRows.results ?? []) {
    ...
    } else if (action.action === "resolve" || action.action === "reopen") addAction(action.comment_id, action.action, action.actor_email);
  }
  for (const [commentId, feedback] of latestFeedback)
    for (const [actorEmail, action] of feedback) addAction(commentId, action, actorEmail);
  const authorEmails = [
    ...new Set(
      [
  ...
  const commentsWithSelections = commentRows.map((comment) => {
    ...
    if (comment.acknowledged_by && !entry.actorCounts.acknowledge.has(comment.acknowledged_by.toLowerCase()) && !entry.actorCounts.unacknowledge.has(comment.acknowledged_by.toLowerCase())) addAction(comment.id, "acknowledge", comment.acknowledged_by.toLowerCase());
...
async function updateEditorialCommentStatus(
  ...
  const latestAction = await env.REPORTS.prepare(
    ...
    .first<{ action: string }>();
  if ((action === "acknowledge" || action === "unacknowledge") && latestAction?.action === action)
